...
namespace IB.Core.SqlQueryBuilder.Clauses
{
    public class WhereStatement : List<List<WhereClause>>
    {
        ...
        public WhereClause Add(System.Enum field, Comparison @operator, object compareValue) { return this.Add(field.ToString(), @operator, compareValue, 1); }
        public WhereClause Add(string field, Comparison @operator, object compareValue, int level)
        {
            WhereClause NewWhereClause = new WhereClause(field, @operator, compareValue,level);
                    ...
                    if (Clause.SubClauses != null)
                    {
                            ...
                            // Loop through all subclauses, append them together with the specified logic operator
                        {
                            ...
                            if (useCommandObject)
                            {
                                ...
                                // Create a where clause using the parameter, instead of its value
                                WhereClause += CreateComparisonClause(Clause.FieldName,
                                    SubWhereClause.ComparisonOperator, new SqlLiteral(parameterName));
                            }
                            else
                            {
                                WhereClause += CreateComparisonClause(Clause.FieldName,
                                    SubWhereClause.ComparisonOperator, SubWhereClause.Value);
                            }
                        }
                    }
        ...
        public static WhereStatement Copy(WhereStatement statement)
        {
            ...
            foreach (List<WhereClause> level in statement)
            {
                ...
                {
                    WhereClause clauseCopy = new WhereClause(clause.FieldName, clause.ComparisonOperator, clause.Value,currentLevel);
